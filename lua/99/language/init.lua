...
function M.initialize(_99)
    ...
    for _, lang in ipairs(_99.languages) do
        ...
        assert(type(M.languages[lang].add_function_spacing) == "function", "add_function_spacing not provided by language")
    end
end
...
function M.add_function_spacing(_99, location)
    ...
    print("language", "file_type", vim.inspect(location.file_type), "lang", vim.inspect(lang))
    if not lang then
        Logger:fatal("langauge currently not supported", "lang", lang)
    end

    if type(lang.add_function_spacing) ~= "function" then
        Logger:fatal("language does not support add_function_spacing", "lang", lang)
    end

    if _99.ai_stdout_rows == 0 then
        Logger:debug("ai_stdout_rows is 0, so no ai indicators will be displayed inline")
        return -1
    end
